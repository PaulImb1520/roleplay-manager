...
export class CycleAlternativeUseCase {
  constructor(
    private readonly conversationRepository: ConversationRepository,
    private readonly messageRepository: MessageRepository,
  ) {}

  async execute(input: CycleAlternativeInput): Promise<MessageDTO> {
    ...
    const cycled =
      input.direction === "prev" ? message.cyclePrev() : message.cycleNext()
    await this.messageRepository.update(cycled)

    return toMessageDTO(cycled)
  }
}
